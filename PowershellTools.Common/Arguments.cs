...
namespace PowerShellTools.Common
{
    ...
    internal static class Arguments
    {
        ...
        public static void ValidateExactlyOneNotNull<T>(T value1, string name1, T value2, string name2) where T : class
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.ValidateExactlyOneSpecifiedFailed_2args, name1, name2));
            }
        }
        ...
        public static T Validate<T>(T value, Func<T, bool> isValid, string name)
        {
            if (!isValid(value))
            {
                throw new ArgumentException(Resources.ArgumentException_1arg.FormatCurrentCulture(name));
            }
