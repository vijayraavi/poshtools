...
namespace PowerShellTools.Repl
{
    ...
    public class ReplWindow : ToolWindowPane, IOleCommandTarget, IReplWindow, IVsFindTarget
    {
        ...
        {
            Debug.Assert(!_isRunning, "Should not allow remote session exit while there is an ongoing execution.");

            UIThread(() =>
            {
                _isRunning = true;
                Task<ExecutionResult> task = Evaluator.ExitRemoteSession();
        ...
        {
            Debug.Assert(!_isRunning, "Should not allow remote session entering while there is an ongoing execution.");
            ...
            UIThread(() =>
            {
                ...
                if (view.ShowModal() == true)
                {
                    ...
                    Task<ExecutionResult> task = Evaluator.EnterRemoteSession(computerName);
                    task.ContinueWith(FinishExecute, _uiScheduler);
                }
            });
        }
